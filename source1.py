...
class InsiderDataset(Dataset):
    def __init__(self, csv_path, seq_len=20):
        ...
        self.file_tree_encoder = LabelEncoder()
        self.activity_encoder = LabelEncoder()

        df["user"] = self.user_encoder.fit_transform(df["user"].astype(str))
        df["pc"] = self.pc_encoder.fit_transform(df["pc"].astype(str))
        df["file_tree"] = self.file_tree_encoder.fit_transform(df["file_tree"].astype(str))
        df["activity"] = self.activity_encoder.fit_transform(df["activity"].astype(str))

        # Convert date to timestamp in seconds (fixed for pandas 2.x)
        df["timestamp"] = pd.to_datetime(df["date"], errors="coerce").view("int64") // 10**9
        df["timestamp"] = df["timestamp"].fillna(0)  # replace NaT with 0 if any

        # Scale features
        scaler = MinMaxScaler()
        df[["user", "pc", "file_tree", "activity"]] = scaler.fit_transform(
            df[["user", "pc", "file_tree", "activity"]]
        )

        # Group sequences by 'id'
        grouped = []
        for _, group in df.groupby("id"):
            features = group[["user", "pc", "file_tree", "activity"]].values
        ...
        #  FIX 1: Faster conversion to tensor (no slow creation warning)
        self.sequences = torch.tensor(np.array(self.sequences), dtype=torch.float32)

        # Placeholder binary labels (replace with actual insider threat labels)
...
class TSTransformer(nn.Module):
    ...
    def forward(self, x):
        features = x[:, :, :-1]  # exclude time diff
        time_diffs = x[:, :, -1]
...
class ActivityGraph(nn.Module):
    ...
    def forward(self, x):
        #  FIX 2: Ensure single sequence graph to avoid GCNConv index error
        if x.dim() > 2:  # batched input
            x = x[0]  # take the first sequence for graph construction
...
class HybridLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, margin=1.0, lambda_graph=0.1):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.margin = margin
...
def train_model(csv_path, epochs=5, batch_size=16, lr=1e-3):
    ...
    model = InsiderThreatModel(input_dim=5)  # 4 features + time_diff
    loss_fn = HybridLoss()
...
if __name__ == "__main__":
    train_model(r"D:\MAHESH\time\12841247\r5.1\r5.1\device.csv", epochs=10)
